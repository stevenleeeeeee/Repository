```txt
Xtrabackup是由percona提供的mysql备份工具，也是惟一一款开源的能够对innodb和xtradb数据库进行热备的工具
...
--host=                   指定备份主机
--socket=                 指定socket文件路径
--databases=              备份指定库,多个空格隔开，如--databases="dbname1 dbname2"（默认备份所有库）
...
[root@localhost /]# innobackupex --stream=tar  /backup | ssh \
    user@xxx.com  "cat - > /backups/`date +%F_%H-%M-%S`.tar" 

# 在执行本地备份时还可用'--parallel'对多个文件进行并行复制。此选项用于指定在复制时启动的线程数
# 实际备份时要利用其便利性则需启用innodb_file_per_table或共享表空间通过innodb_data_file_path选项存储在多个ibdata文件中
...
#### 备份、恢复操作流程相关参考
```bash
https://www.cnblogs.com/liangshaoye/p/5464794.html
https://www.cnblogs.com/crysmile/p/9471565.html
https://blog.csdn.net/wanglei_storage/article/details/49430515
```