```bash
# ZooKeeper3.4.6支持某些特定的四字命令字母与其的交互。它们大多是查询命令，用来获取 ZooKeeper 状态相关的信息。
# 用户在客户端可通过 telnet 或 nc 向 ZooKeeper 提交相应的命令 
# 其中stat、srvr、cons三个命令比较类似：
...
#命令    描述
conf	# 输出相关服务配置的详细信息。如端口、zk数据及日志配置路径、最大连接数，session超时、serverId等
cons	# 所有连接到这台服务器的客户端连接/会话的信息。包括"接受/发送"包数量、session id 、操作延迟、最后的操作执行等信息
crst	# 重置当前这台服务器所有连接/会话的统计信息
dump	# 列出未经处理的会话和临时节点（只在leader有效）。
envi	# 输出关于服务器的环境详细信息（不同于conf命令），如host.name、java.version、java.home、user.dir=/data/zookeeper-3.4.6/bin之类信息
...
wchc	# 通过session分组，列出watch的所有节点，其输出是一个与 watch 相关的会话的节点列表。如果watches数量很大的话将会产生很大的开销，会影响性能
wchp	# 通过路径分组列出所有的 watch 的session id信息。它输出与 session 相关的路径。如果watches数量很大的话，将会产生很大的开销，会影响性能
mntr	# 列出集群的健康状态。包括"接受/发送"的包数量、操作延迟、当前服
```
#### 输出说明
```txt
conf:
