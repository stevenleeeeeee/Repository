...
class student(object):
    '''some information ...'''          #使用特殊属性"__doc__"（钩子）访问注释
...
#类的三种方法：普通方法、静态方法、类方法。它们在内存中都归属于类!! 区别在于调用方式的不同：
 ...
        print "I'm Good boy"
    ...
    @staticmethod                       #静态方法基本上跟一个全局函数相同，一般来说用的很少（静态方法与静态字段个对象没关系）
    def sayHello(hi):			#静态方法好处是不需定义实例即可使用它，另外实例之间共享静态方法!
        if hi is None:			#它不能对实例对象的数据进行操作（它几乎和类与对象没有任何的联系）
            hi = 'hello'
        print(hi)
