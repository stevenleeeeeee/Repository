...
```txt
SQLAlchemy：
对象关系映射代表了用户使用Python定义类来与数据库中的表相关联的一种方式，类的实例对应数据表中一行数据
其包括了一套将对象中的变化同步到数据库表中的系统，这套系统被称之为工作单元
同时也提供了使用类查询来实现数据库查询及查询表间关系的功能

SQLAlchemy　ORM与SQLAlchemy表达式语言（SQLAlchemy Expression Language）是不同的：
前者是在后者的基础上构建的，也就是说是基于后者实现的（将后者封装，类似于实现一套API）。
它代表了关系数据库最原始的一种架构，而SQLAlchemy ORM则代表了一种更高级也更抽象的实现，同时也是SQL表达式语言的应用

尽管看起来ORM和表达式语言使用起来有点相似，但是其相似点比他们乍一看起来还要少
一种方式是通过透视用户定义的领域模型来实现数据的内容与结构
另外一种方式通过文字模式（ literal schema）和SQL表达式来明确的操作数据库

一个成功的应用可能只使用对象关系映射的构造。在更复杂的情况下，ORM及SQL表达式语言互相配合使用也是必不可少的
```
#### Version Check
```python
# 安装SQLAlchemy过程，建议先安装 [virtualenv] 再使用pip安装
...
# echo用来设置SQLAlchemy日志，通过Python标准库logging模块实现
# 为True时可以看见所有的SQL操作记录，可设为False来减少日志输出
# create_engine() 返回值是 Engine 的实例，此实例代表了操作数据库的核心接口，通过方言来处理数据库和数据库的API
# 在本例中，SQLite方言将被翻译成Python内置的sqlite3模块
# 个人理解方言指的是每一种数据库的使用的方言，比方说mysql会有一种，sqlite又会有一种

# 当第一次调用 Engine.execute() 或 Engine.connect() 方法时 Engine 会和数据库建立真正的DBAPI连接，用来执行SQL
# 但在创建连接后我们很少直接使用Engine来操作数据库，更多的会使用ORM这种方式来操作

# 什么是 Database Urls: http://docs.sqlalchemy.org/en/latest/core/engines.html#database-urls

# Lazy Connecting: 
# 初次调用 create_engine() 时并不会真正的去连接数据库，只有在真正执行一条命令的时候才会去尝试建立连接，目的是节省资源
# 很多地方都会使用这种方式，如Python中的 lazy property: https://segmentfault.com/a/1190000005818249
```
#### Declare a Mapping
```python
# 当使用ORM时配置过程以描述数据库的表开始，然后定义与之匹配的类
...
class User(Base):
    '''
    users 表的信息包括表名，姓名，还有列的数据类型
    users 表记录了用我们应用的终端用户的信息。与之对应的类称之为User
    '''
    __tablename__ = 'users'                     # 表明

    id = Column(Integer, primary_key=True)      # 定义列
    name = Column(String)                       #
...
# 注：
# User class定义了 __repr__() 函数，这非必须。使用它是为了使例子（User对象）有更好看的格式
...
# 同时也提供读取表中数据的方式，除此之外还有一些普通的Python类，可以让我们定义一些用于应用的一般的参数
```

#### Create a Schema
```python
# 通过Declarative系统构建类之后，我们也定义了被称之为表的元数据信息
...
# 当声明类时 Declarative系统使用Python元类来实现，这样就可以在类被声明之后还可以执行额外的操作
# 在这个过程中会根据我们的指定创建一个Table对象，然后通过构建一个Mapper对象使之与类关联。
# 这个对象是个幕后的对象，以至于我们一般不直接与它打交道（虽然在我们需要的时候它也能我们提供很多信息）
# Table 对象是一系列的元数据的组合。当使用Declarative时，这个对象可以使用我们declarative的基类的 .metadata 属性
...
# 所以如果使用了其他的数据库同时想使用SQLAlchemy来创建一个表，那么可能需要给String type添加一个长度：
Column(String(50))
...
# __init__() 方法:
# 使用Declarative系统创建的类包含了一个构造函数，可以自动的接受并匹配关键词，当然也可定义 __init__() 函数，这样就会覆盖
...
# 当第一次创建这个应用的时候，使用 create_engine() 同时也定义一个Session类来当做一个工厂来处理Session对象:
...
# PS:
# 会话的生命周期模式，对话只是对象指向一个数据库链接的一个工作空间
```
#### 添加／更新对象 （Adding and Updating Objects）
```python
# 为了持续操作 User 类需持久化的对象，我们把他添加 add() 到会话中：
>>> ed_user = User(name='ed', fullname='Ed Jones', password='edspassword')
>>> session.add(ed_user)
# 现在称这个对象是待定的（pending）
...
# 一旦一个对象已经在会话中有个主键（primary key），所有关于这个key的SQL查询都只返回一个同样的Python对象
...
# 通知会话想保留所有的更改然后提交到数据库。使用 commit() 提交这些更改
# 会话提交 UPDATE 来更改 ed 的密码，同时用 INSERT 实现新的对象的插入：
>>> session.commit()
...
# 参考：http://docs.sqlalchemy.org/en/latest/orm/session_state_management.html#session-object-states
```

#### 回滚（Rolling Back）
```python
# 既然会话在一个事务里边起作用，那么也可以在这个事务里回滚一些变化
# 让我们做两个更改然后再删掉更改：把 ed_user 的用户名改成 Edwardo 
...
# 回滚一下，我们就看到ed_user的用户名已经改回了ed，假用户也不存在了：
>>> session.rollback()
>>> ed_user.name
# u'ed'
>>> fake_user in session        # 注，这里是 __contains__() 魔法方法实现的 in 操作
# False
```
#### Quering
```python
...
# 当我们执行这些语句时可以看到 User 对象返回来的查询列表：
...
# Query 也接受ORM内置的描述符当参数，任何时候多个类实体或基于列的实体也可作为查询方法 query 的参数，返回如下：
...
# Query 返回的结果称之为元组，通过 KeyedTuple class实现，同时可以被当做Python的原生对象来处理。
...
>>> for row in session.query(User.name.label('name_label')).all():      # 查询时指定别名
...    print(row.name_label)                                            # 输出时使用别名
ed 
wendy
mary
fred
...
# 但若有两个参数呢? query()里有两个参数的情况，上面的例子只有一个）？可使用 aliased() 来解决（和bash里的alias差不多）:
...
# Query 基本操作包括 LIMIT 和 OFFSET，但更简单的是切片或 ORDER_BY：
...
# Query 对象是完全可繁殖的，意味着大多数方法的调用都返回一个新的 Query 对象，此对象仍可进行查询操作
# 例如可以调两次 filter() 函数来查用户名为ed并且全名为Ed Jones的用户，相当于SQL的AND操作：
...
#### Using Textual SQL
```python
# Query 可非常灵活的使用一些字面的字符，通过 text 构造来实现，同时可与常见方法结合使用，如：filter()、order_by()
...
# 使用基于字符串的SQL可通过冒号来指定参数。为参数赋值可使用 params()：
>>> session.query(User).filter(text("id<:value and name=:name")).params(value=224, name='fred').order_by(User.id).one()
# <User(name='fred', fullname='Fred Flinstone', password='blah')>

# 为了使用完全的SQL字串，可使用 from_statement() 实现。不需额外的指定
...
# 在简单的例子中去根据名字匹配是可行的，但当处理包含着重复的名字的复杂语句或使用匿名的ORM架构的时候就不太现实了
