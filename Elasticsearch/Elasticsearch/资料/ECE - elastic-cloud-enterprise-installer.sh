...
# ECE version
CLOUD_ENTERPRISE_VERSION=2.0.1
...
# Hardcode the DNT feature flag to true for 2.0.0 and up
export CLOUD_FEATURE_FLAG_DEDICATEDNODETYPES=true
...
case $COMMAND in
  ...
  add-stack-version )               shift
                                    ;;
  set-logging-and-metrics-policy )  shift
                                    ;;
  upgrade )                         shift
                                    ;;
  --help|help )
      ...
      echo ""
      echo "  set-logging-and-metrics-policy Sets the index retention period for logging and metrics indices"
      echo ""
...
# Otherwise, it assigns the argument's value to a variable that name is passed as the first parameter
# Parameters:
...
# reserved range of the 0-1000) in order to avoid UID collision with
# the internal docker image's users. Additionally, verify that the
...
validateRunningUser(){
  ...
  if [ -z "$docker_group" ]; then
    echo -e "${RED}The user is not a member of the docker group.${NC}"
  ...
  while [ "$1" != "" ]; do
    case $1 in
      ...
      --help|help)
                        ...
                        echo "[--roles-token TOKEN] [--roles \"ROLES\"] [--force]"
                        echo ""
                        echo "Arguments:"
                        ...
                        echo "                           makes available for Elasticsearch clusters. Must be "
                        echo "                           at least 8192 MB."
                        echo "                           Defaults to 85% of available RAM if not specified."
                        echo ""
...
parseResetAdminconsolePasswordArguments() {
  ...
  while [ "$1" != "" ]; do
    case $1 in
        ...
        --help|help )     echo "============================================================================================="
                          ...
                          echo "[--pwd NEW_PASSWORD] [--host-storage-path PATH_NAME] [--secrets PATH_TO_SECRETS_FILE]"
                          echo "[[--]help]"
                          echo ""
                          echo "Arguments:"
                          echo ""
  ...
  if [[ -z "$BOOTSTRAP_SECRETS" ]]; then
      ...
      ZK_ROOT_PASSWORD=$(docker -H "unix://${HOST_DOCKER_HOST}" exec -it frc-directors-director bash -c 'echo -n $FOUND_ZK_READWRITE' 2>/dev/null | cut -d: -f 2)
      if [[ -z "$ZK_ROOT_PASSWORD" ]]; then
        echo -e "${RED}Failed to get access to Elastic Cloud Enterprise."
        echo -e "Please meet at least one of the following requirements:${NC}"
...
parseUpgradeArguments() {
  ...
  while [ "$1" != "" ]; do
    case $1 in
        ...
        --help|help )     echo "=========================================================================================="
                          ...
                          echo "[--overwrite-existing-image] [--debug] [[--]help]"
                          echo ""
                          echo "Arguments:"
                          echo ""
                          echo ""
  ...
  SOURCE_CONTAINER_NAME="frc-runners-runner"
  HOST_STORAGE_PATH=$(docker -H "unix://${HOST_DOCKER_HOST}" exec -it $SOURCE_CONTAINER_NAME bash -c 'echo -n $HOST_STORAGE_PATH' 2>/dev/null | cut -d: -f 2)
  ...
  ZK_ROOT_PASSWORD=$(docker -H "unix://${HOST_DOCKER_HOST}" exec -it $SOURCE_CONTAINER_NAME bash -c 'echo -n $FOUND_ZK_READWRITE' 2>/dev/null | cut -d: -f 2)
  if [[ -z "${ZK_ROOT_PASSWORD}" ]]; then
...
resetAdminconsolePassword() {
  CLOUD_IMAGE=$(docker -H "unix://${HOST_DOCKER_HOST}" inspect -f '{{ .Config.Image }}' $SOURCE_CONTAINER_NAME)
  ...
  if [[ ! -z "${CLOUD_IMAGE}" ]]; then
    docker -H "unix://${HOST_DOCKER_HOST}" run \
        --env ZK_AUTH=$ZK_ROOT_PASSWORD \
        $(docker -H "unix://${HOST_DOCKER_HOST}" inspect -f '{{ range .HostConfig.ExtraHosts }} --add-host {{.}} {{ end }}' $SOURCE_CONTAINER_NAME) \
        $MNT \
...
addStackVersion() {
  ...
  if [[ ! -z "${CLOUD_IMAGE}" ]]; then
    if [[ -e "${VERSION}.zip" ]]; then
      ...
      ADD_STACKPACK_RESULTS=$(docker -H "unix://${HOST_DOCKER_HOST}" run \
        --env USER=$USER \
        --env PASS=$PASS \
        ...
        --rm -it $CLOUD_IMAGE \
        bash -c 'wget -q -O - --content-on-error \
...
parseSetLoggingAndMetricsPolicyArguments() {
  SOURCE_CONTAINER_NAME="frc-runners-runner"
  ZK_ROOT_PASSWORD=""
  PRINT=""
  PATTERN=""
  ...
  while [ "$1" != "" ]; do
    case $1 in
        ...
        --print )                     PRINT="--print true"
                                      ;;
        --pattern )                   setArgument PATTERN $1 $2
                                      PATTERN="--pattern $PATTERN"
                                      shift
                                      ;;
        --days )                      setArgument DAYS $1 $2
                                      DAYS="--days $DAYS"
                                      shift
                                      ;;
        --host-storage-path )         setArgument HOST_STORAGE_PATH $1 $2
                                      ...
                                      ;;
        --help|help )     echo "======================================================================================="
                          echo "Set the logging and metrics index retention period."
                          echo "The script should be run on either the first host you installed Elastic Cloud"
                          echo "Enterprise on or a host that holds the director role."
                          echo ""
                          echo "${0##*/} set-logging-and-metrics-policy [--host-docker-host HOST_DOCKER_HOST] [--print]"
                          echo "[--pattern PATTERN] [--days DAYS] [--host-storage-path PATH_NAME]"
                          echo "[--secrets PATH_TO_SECRETS_FILE] [[--]help]"
                          echo ""
                          echo "Arguments:"
                          echo ""
                          echo "--host-docker-host   Set the docker's docker-host location"
                          echo "                     Defaults to /var/run/docker.sock"
                          echo ""
                          echo "--print              Print the policy that is currently in effect. This option"
                          echo "                     cannot be specified with --pattern and --days."
                          echo ""
                          echo "--pattern            Specifies the pattern (e.g. 'cluster-logs-*') for which"
                          echo "                     a new retention period will be set. If this option is"
                          echo "                     present, --days must also be provided."
                          echo ""
                          echo "--days               Specifies the number of days that indices matching the"
                          echo "                     given pattern will be kept (0 or -1 will disable index"
                          echo "                     curation and result in indices being retained forever)."
                          echo "                     If this option is present, --pattern must also be provided."
                          echo ""
                          echo "--host-storage-path  Specifies the host storage path used by the Elastic Cloud"
                          echo "                     Enterprise installation. It is used for calculating"
                          echo "                     a location of the default file with secrets as well as"
                          ...
                          echo ""
                          echo "Examples:"
                          echo "${0##*/} set-logging-and-metrics-policy --pattern "cluster-logs-*" --days 14"
                          echo ""
                          exit 0
                          ;;
       *)  echo -e "${RED}Unknown argument '$1'${NC}"
...
setLoggingAndMetricsPolicy() {
  ...
  if [[ ! -z "${CLOUD_IMAGE}" ]]; then
    docker -H "unix://${HOST_DOCKER_HOST}" run \
        ...
        -v "$HOST_STORAGE_PATH/logs":"/app/logs" \
        --rm -it $CLOUD_IMAGE \
        /elastic_cloud_apps/bootstrap/set_logging_and_metrics_policy/set-logging-and-metrics-policy.sh $PRINT $PATTERN $DAYS $SECRETS_ARG # run directly, bypass runit
  else
      echo -e "${RED}Container $SOURCE_CONTAINER_NAME was not found -- is the environment running?${NC}"
      exit $GENERAL_ERROR_EXIT_CODE
  fi
...
runBootstrapInitiatorContainer() {
  ...
  if [ -n "${RUNNER_EXTERNAL_HOSTNAME}" ]; then
      DOCKER_ADDITIONAL_ARGUMENTS="--env RUNNER_EXTERNAL_HOSTNAME=${RUNNER_EXTERNAL_HOSTNAME} ${DOCKER_ADDITIONAL_ARGUMENTS}"
  fi
...
pullElasticCloudEnterpriseImage() {
  ...
  docker -H "unix://${HOST_DOCKER_HOST}" pull ${DOCKER_REGISTRY}/${ECE_DOCKER_REPOSITORY}/elastic-cloud-enterprise:${CLOUD_ENTERPRISE_VERSION}
}
  ...
  while [ "$1" != "" ]; do
    case $1 in
        ...
        --help|help )     echo "================================================================================"
                          ...
                          echo "[--user USER_NAME] [--pass PASSWORD] [--version A.B.C] [[--]help]"
                          echo ""
                          echo "Arguments:"
                          echo ""
                          echo "--host-docker-host   Set the docker's docker-host location"
...
main() {
  ...
  if [ $COMMAND == "install" ]; then
    ...
    parseStackVersionArguments "$@"
    addStackVersion
  elif [ $COMMAND == "set-logging-and-metrics-policy" ]; then
    parseSetLoggingAndMetricsPolicyArguments "$@"
    setLoggingAndMetricsPolicy
