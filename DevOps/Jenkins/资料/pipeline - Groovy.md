...
```txt
# 通过Groovy脚本抓取Git仓库所有Branch并作为选项，方便在Build前去选择Branch
...
#### .....
```txt
def ver_keys = [ 'bash', '-c', 'cd /gitrepos/project1; git pull>/dev/null;
                 git branch -a|grep remotes|grep release|cut -d "/" -f3|sort -r |head -10 ' ]
